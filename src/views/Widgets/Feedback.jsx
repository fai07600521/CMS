...
class ReactTables extends React.Component {
  ...
  createInfo(feedback) {
    ...
    return feedback.map((prop, key) => {
    console.log("prop.gbbookingId")
    console.log(prop)
    console.log(prop.gbbookingId)
      // eslint-disable-next-line react/prop-types
      ...
      return {
        ...
        bookingID: (prop.gbBookingId) ?prop.gbBookingId : "-",
        bookingDate:(prop.pickUpTime) ? moment(prop.pickUpTime).format('DD/MM/YYYY HH:mm:ss'): "-",
        comment:  (prop.comment) ? prop.comment : "-",
        title : (prop.title) ? prop.title : "-",
        rate: <Rater rating={prop.rate} total={5} interactive={false} />,
        passengerName: (prop.passengerName) ? prop.passengerName : "-",
        actions: (
  ...
  async componentWillMount() {
    await this.props.feedback.initData();
    ...
    let confirmDialogOptions = {
      ...
      preConfirm: async () => {
        try {
          ...
          await this.props.feedback.deleteElement(index);
          result === 200 &&
            (await Swal.fire({
              type: "success",
  ...
  render() {
    const { feedback } = this.props.feedback.toJS();
    const info = this.createInfo(feedback);
...
export const page = inject("feedback")(observer(ReactTables));
